...
class TestSeleniumTutorialStyleSuite(BaseSeleniumIntegrationTest):
	...
	def test_05_friend_search_results(self):
		unique = int(time.time() * 1000)
		user_a_email = f"friend_a_{unique}@example.com"
		user_b_email = f"friend_b_{unique}@example.com"
		...
		# Login as Alice and go to friends page
		self._login(user_a_email)
		self.driver.get(f"{self.base_url}/friends")

		# Search for Bob by name and verify results
		search_input = self.wait.until(EC.presence_of_element_located((By.ID, "friendSearch")))
